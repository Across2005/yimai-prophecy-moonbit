...
[![Service API](https://img.shields.io/badge/HTTP%20API-22%20endpoints%20%2B%20MCP-9cf)](https://github.com/Across2005/yimai_prophecy_moonbit)
...
- **Pure-MoonBit service layer (纯 MoonBit 全栈)** — `cmd/service` 起本地 HTTP server（`127.0.0.1:8787`），**22 个 `/api/*` 端点 + `/mcp` MCP Server**（13 基础 + retrieve_prompt / bleu / chrf / style_check / back_align / term_conflicts / fed_export / fed_import / distill_inject）全部实测通过，含 BLEU/chrF++ 评测、风格检查、回译对齐、术语冲突、TMPlm、联邦/蒸馏注入；TM 状态经 fs **原子写持久化**（tmp+rename），重启恢复闭环——**引擎到服务零桥接语言**。
...
| **对外接口** | 仅 MoonBit 函数调用（`moon add` 后进程内调用） | **22 个 HTTP REST 端点**，前端 / Agent / LLM 宿主可直接消费 |
...
| `/api/back_align` | POST | `{"source","target"}` | `{align_score,misaligns}` | 回译 LCS 对齐验证 |
...
22 个端点不是孤立的——它们把三层连成**记忆闭环**（完整映射表见架构方案 §11）：
...
> **安装为 WorkBuddy skill（可选）**：仓库内 [`docs/skill/SKILL.md`](./docs/skill/SKILL.md) 是编排手册（frontmatter 含 `agent_created: true`，触发词 + 22 端点 API 手册 + MCP 接入 + 数据契约）。复制到 `~/.workbuddy/skills/yimai-prophecy/` 并重启 WorkBuddy 后即成为可用技能。
