...
| Layer 2 · service | `cmd/service/` | 13 REST endpoints (`/api/*`) + `/mcp` MCP server + web workbench |
...
```bash
moon test --target wasm-gc        # 99/99 green
```

## Consuming the service

- **REST**: `POST /api/fuzzy_match {"query":"电池包热管理方案","k":3,"threshold":0.5}` → Top-K with
  white-box scores (`sim_token/sim_tfidf/sim_char/sim_ngram/sim_tokenset`). All 13 endpoints are
  `POST /api/*` with JSON bodies (see `README.md` → Service Layer).
- **MCP**: `POST /mcp` speaks JSON-RPC 2.0 (spec 2025-11-25). `initialize` → `tools/list` (13 tools)
