...
| Layer 2 · service | `cmd/service/` | 22 REST endpoints (`/api/*`) + `/mcp` MCP server + web workbench |
...
- **REST**: 22 个 `/api/*` 端点（多数 `POST` JSON 体，`/api/ping` 与 `/api/tm_count` 为 GET）。示例
...
- **MCP**: `POST /mcp` speaks JSON-RPC 2.0 (spec 2025-11-25). `initialize` → `tools/list` (22 tools)
